    ...
    def post(self):
        try:
            ...
            SELECT
            	...
            	, ONOFF
            	, ENERGY
            	, ENERGY * (SELECT TIMESTAMPDIFF(MINUTE, START_DATE, END_DATE) FROM AH_DR_REQUEST WHERE REQUEST_DR_NO = '{request_dr_no}') as ENERGY_SUM
            FROM
            (SELECT
            	FR.DEVICE_ID
            	, T.DEVICE_NAME
            ...
            FROM (
            	...
            	INNER JOIN 
            		...
            		GROUP BY
            			gateway_id
            			, device_id
            			, device_name) T on (FR.DEVICE_ID = T.DEVICE_ID)) FF
            ORDER BY
            	STATUS desc
            	, ONOFF desc
            	, FREQUENCY desc
